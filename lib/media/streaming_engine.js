...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  // This is a client-side hack around a problem with DASH content. Please
  // see discussion in DFWAVPTEAM-15281.
  // Ignore presentationTimeOffset when calculating offset for text tracks
  // which are not ISOBMFF-packaged.
  // Otherwise, these text tracks are sent way into the past and get dropped.
  // (Using PTO with segmented plain text tracks is not supported in DASH,
  // hence why Shaka does not support it.)
  if (mediaState.type == ContentType.TEXT &&
      mediaState.stream.mimeType != 'application/mp4') {
    timestampOffset = currentPeriod.startTime;
  } else {
    timestampOffset =
